...
class Item {
    ...
    static getImageAsset(name, type, found) {return `assets/images/${ found
...
class ProgressTracker {
    ...
    async #readItemsJSON() {
        try {
            let res = await fetch("data/base_itemsByRegion.json");
            const itemsData1 = await res.json();
            let res2 = await fetch("data/sote_itemsByRegion.json");
            const itemsData2 = await res2.json();
            this.itemData = { ...itemsData1, ...itemsData2 };
            res = await fetch("data/allCollectibles.json");
